class Solution:
    def candyCrush(self, board):
        ...
        """
        while True:
            candies = self.crash(board)
            print(candies)
            if not candies:
                break
            self.fall(board, candies)
        return board
    ...
    def crash(self, board):
        ...
        col = len(board[0])
        candies = set()
        # crash candies horizontally,
        for r in range(row):
            for c in range(col-2):
                if board[r][c] != 0 and board[r][c] == board[r][c+1] and board[r][c+1] == board[r][c+2]:
                    candies.add((r, c))
                    candies.add((r, c+1))
                    candies.add((r, c+2))
        # crash candies vertically.
        for c in range(col):
            for r in range(row-2):
                if board[r][c] != 0 and board[r][c] == board[r+1][c] and board[r+1][c] == board[r+2][c]:
                    candies.add((r, c))
                    candies.add((r+1, c))
                    candies.add((r+2, c))
        return candies

    def fall(self, board, candies):
        row = len(board)
        col = len(board[0])
        ...
        for c in range(col):
            ...
            rr = wr
            while rr > -1:
                if (rr, c) not in candies:
                    board[wr][c] = board[rr][c]

                    rr -= 1
                    wr -= 1
                else:
                    rr -= 1
            # filled 0 for top empty cells
            while wr > -1:
                board[wr][c] = 0
                wr -= 1
        return

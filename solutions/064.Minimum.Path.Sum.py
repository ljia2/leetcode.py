class Solution:
    def minPathSum(self, grid):
        ...
        dp[i][j] denotes the min summed path from grid[0][0] to grid[i-1][j-1]

        dp[i][j] = min(dp[i-1][j] + grid[i-1][j-1], dp[i][j-1] + grid[i-1][j-1]dp

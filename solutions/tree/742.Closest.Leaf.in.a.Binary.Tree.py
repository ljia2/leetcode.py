...
class Solution:
    def findClosestLeaf(self, root, k):
        ...
        How about converting tree to undirected graph and then bfs from target node.
    ...
    def dfs(self, root, graph, leaf):
        ...
        if root.left:
            ...
            self.dfs(root.left, graph, leaf)

        if root.right:
            graph[root.val].append(root.right.val)
            graph[root.right.val].append(root.val)
            self.dfs(root.right, graph, leaf)
        return

...
class Solution(object):
    def mincostTickets(self, days, costs):
        ...
        dp[i][0] : the min cost of traveling at days[i] by a 1 day pass.
        dp[i][1] : the min cost of traveling at days[i] by a 7 day pass.
        dp[i][2] : the min cost of traveling at days[i] by a 30 day pass.


        """

        tcosts = [[0] * 3 for _ in range(len(days))]
        tcosts[0][0] = costs[0]
        tcosts[0][1] = costs[1]
        tcosts[0][2] = costs[2]
        ...
        for d in range(1, len(days)):
            ...
            tdi = bisect.bisect_right(days, td)
            if tdi > 0:
                tcosts[d][2] = min(tcosts[tdi-1][0], min(tcosts[tdi-1][1], tcosts[tdi-1][2])) + costs[2]
            else:
                tcosts[d][2] = costs[2]

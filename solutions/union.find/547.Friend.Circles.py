    ...
    def findCircleNum(self, M):
        ...
        for r in range(len(M)):
            for c in range(r+1, len(M[0])):
                if M[r][c] == 1:
                    pr = self.find(r, parents)
                    pc = self.find(c, parents)

                    if pr == pc:
                        continue

                    if ranks[pr] > ranks[pc]:
                        pr, pc = pc, pr
        ...
        components = set()
        # for each people, find their parents and record the unique parents
        for r in range(n):
            components.add(self.find(r, parents))

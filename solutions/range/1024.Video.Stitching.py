class Solution(object):
    def videoStitching(self, clips, T):
        ...
        minimum number of clips hints DP.

        1 <= clips.length <= 100 and 0 <= clips[i][0], clips[i][1] <= 100 hint O(n^3)


        dp[i][j] the minimum clips covering time interval [s, e]
        return dp[0][T]
            ...
            for l in range(1, T+1):
                for i in range(T-l+1):
                    ...
                    if s <= i and j <= e:
                        ...
                        dp[i][j] = min(dp[i][j], dp[i][s] + dp[e][j] + 1)

        return -1 if dp[0][T] == n + 1 else dp[0][T]
